...
|----------|-----|------------------------|
| **Spring WebFlux (Reactive)** | Non-blocking I/O handles high transaction throughput with fewer threads. A single instance can handle thousands of concurrent connections without thread-per-request overhead. *Interview tip: Mono and Flux enable non-blocking by representing asynchronous operations as streams. When a Mono/Flux operation starts (e.g., DB query), it returns immediately without blocking the thread. The thread is freed to handle other requests while the operation continues in the background. When the operation completes, the result is pushed through the reactive pipeline using callbacks/continuations, eliminating thread waiting time.* | Spring MVC - simpler but blocks threads waiting for DB/Kafka responses |
